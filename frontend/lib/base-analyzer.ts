...
const STYLE_DESCRIPTIONS: Record<BaseStyle, string> = {
  rusher: "大本营等级较高，但建筑/科技进度落后。升级速度快但战力可能不足，建议补齐关键防御和科技。",
  offense_heavy: "进攻科技优先于防御。适合主动出击的玩家，但防守薄弱易被掠夺，建议补强关键防御。",
  defense_heavy: "防御等级高于进攻科技。基地防守扎实，但进攻乏力，建议优先升级兵种和法术。",
  balanced: "进攻与防御均衡发展。基地整体健康，按优先级继续推进即可。",
  maxer: "大部分建筑接近满级。基地发展成熟，可考虑升级大本营解锁新内容。",
};
...
function generateSuggestions(
  ...
  switch (style) {
    case "rusher":
      suggestions.push("大本营等级偏高，建议暂停升本，优先补齐关键防御和实验室科技。");
      if (defense && defense.avgProgress < 40) {
        suggestions.push(`防御进度仅 ${defense.avgProgress}%，优先升级防空火箭、X连弩等核心防御。`);
      }
      break;
    case "offense_heavy":
      if (defense && defense.avgProgress < 50) {
        suggestions.push(`防御进度 ${defense.avgProgress}% 偏低，建议投入工人补强法师塔、防空火箭。`);
      }
      break;
    case "defense_heavy":
      if (offense && offense.avgProgress < 50) {
        suggestions.push(`进攻科技 ${offense.avgProgress}% 偏低，优先升级实验室和常用兵种/法术。`);
      }
      break;
    case "maxer":
      suggestions.push("基地发展接近满级，可考虑升级大本营解锁新内容。");
      break;
    case "balanced":
      suggestions.push("进攻与防御均衡，按优先级继续推进即可。");
      break;
  }

  // 资源建议
  if (resource && resource.avgProgress < 40) {
    suggestions.push(`资源建筑进度 ${resource.avgProgress}%，建议升级金矿/圣水收集器保障长期产出。`);
  }

  // 功能建筑建议
  if (utility && utility.avgProgress < 60) {
    suggestions.push("功能建筑（部落城堡/军队营地等）影响部队容量，建议优先升级。");
  }
  ...
  if (snapshot.builderCount > 0 && activeCount < snapshot.builderCount) {
    suggestions.push(`当前有 ${snapshot.builderCount - activeCount} 个工人空闲，建议立即安排新升级。`);
  }
